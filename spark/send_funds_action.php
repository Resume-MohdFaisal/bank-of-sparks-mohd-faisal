<?php
    ...
    $sql =  "SELECT cust_id FROM customer WHERE account_no='".$acno."'" ;
    $result = $conn->query($sql);
    $row = $result->fetch_assoc();
    if (($result->num_rows) > 0) 
 {
    
        $receiver_id = $row["cust_id"];
...
<body>
    <div class="flex-container">
        <div class="flex-item">
            ...
            <?php
            if ($err_no == 3) { ?>
                <p id="info"><?php echo "accont does not exist !\n"; ?></p>
            <?php } ?>
        </div>

        <div class="flex-item">
            <a href="./send_funds.php" class="button">Go Back</a>
        </div>
    </div>
